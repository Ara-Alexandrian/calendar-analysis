    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(f"""
                ...
                ON {settings.DB_TABLE_CALENDAR_FILES} (file_hash);
            """)
...
def save_calendar_file_to_db(filename, file_content):
    ...
    try:
        ...
        logger.info(f"Checking for duplicate file with hash: {file_hash[:10]}...") # Log before SELECT
        with conn.cursor() as cursor:
        ...
        logger.info(f"Duplicate check complete. Found: {existing is not None}") # Log after SELECT
        ...
        # Before inserting, mark only the *currently* active file (if any) as not current
        logger.info(f"Updating existing 'is_current=TRUE' flags in {settings.DB_TABLE_CALENDAR_FILES}...") # Log before UPDATE
        with conn.cursor() as cursor:
            cursor.execute(sql.SQL("""
                UPDATE {}
            ...
            logger.info(f"Update complete.") # Log after UPDATE

            # Now insert the new file as the current one
            logger.info(f"Inserting new calendar file record for '{filename}' with batch ID {batch_id}...") # Log before INSERT
            cursor.execute(sql.SQL("""
                INSERT INTO {}
            ...
            logger.info(f"Insert complete. New file ID: {file_id}") # Log after INSERT

            logger.info(f"Committing transaction for calendar file save...") # Log before COMMIT
            conn.commit()
            logger.info(f"Commit complete.") # Log after COMMIT
