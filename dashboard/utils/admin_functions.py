...
from src.infrastructure.persistence import connection, operations # New import
...
def save_manual_assignment(entry_id, personnel, event_type):
    ...
    try:
        ...
        # Log the assignment
        # Assuming admin_username is in session_state, might need adjustment if not
        admin_user = st.session_state.get("admin_username", "UnknownAdmin")
        logger.info(f"Manual assignment by admin '{admin_user}': Entry ID: {entry_id}, Personnel: {personnel_list}, Event Type: {event_type}")
